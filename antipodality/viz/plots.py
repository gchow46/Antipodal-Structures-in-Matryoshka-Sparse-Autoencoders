"""Figures for score distributions, hierarchy comparisons, and cosine matrices."""
...
def get_transition_color(feature1_level: int, feature2_level: int) -> str:
    ...
    return LEVEL_FALLBACK_COLORS.get(feature1_level, '#D5DBDB')
...
def draw_heatmap(ax: Axes, M, spec: HeatmapSpec):
    ...
    cbar = getattr(heatmap.collections[0], "colorbar", None) if spec.cbar else None
    return heatmap, cbar
...
def hist(ax: Axes, values, bins=50, label_mean=True, label_median=True):
    ...
    if label_mean:
        ...
        ax.axvline(mean, color="red", lw=2, label=f"Mean = {mean:.3f}")
    if label_median:
        median = float(np.median(values))
        ax.axvline(median, color="orange", ls="--", lw=2, label=f"Median = {median:.3f}")
...
def plot_global_antipodality(payload: dict, out_path: str) -> None:
    """Plot density correlations, score distributions, and dense-feature counts."""
    _require(payload, [
    ...
    correlation = payload["correlation_results"]
    ...
    for i, level in enumerate(MATRYOSHKA_LEVELS):
        mask = (level_ids == i)
        if np.any(mask):
            ax1.scatter(valid_densities[mask], valid_scores[mask], s=25, alpha=0.7,
                        c=LEVEL_COLORS[i], edgecolors="white", linewidth=0.5, label=f"L{level}")
    ax1.axvline(threshold, color="red", ls="--", lw=2, alpha=0.7, label=f"Dense threshold ({threshold:g})")
    ...
    ax1.set_title(f"Density vs Antipodality by Matryoshka Level\n"
                  f"Spearman rho = {correlation['spearman_r']:.3f} (p = {correlation['spearman_p']:.4f})",
                  fontweight="bold")
    ...
    ax4.bar(xs, counts, color=colors, alpha=0.8, edgecolor="black", linewidth=1)
    ...
    ax4.set_xticks(xs)
    ax4.set_xticklabels([str(level) for level in MATRYOSHKA_LEVELS])
    for i, count in enumerate(counts):
        if count > 0:
            ax4.text(xs[i], count + max(0.5, 0.02 * max(counts)), str(count), ha="center", va="bottom", fontweight="bold")
...
def plot_matryoshka_hierarchy(payload: dict, out_path: str) -> None:
    """Plot dense-only antipodality distributions by level."""
...
def plot_enc_dec_scatter(payload: EncDecScatterPayload, out_path: str) -> None:
    """Compare encoder and decoder cosines for the sampled pairs."""
...
def _plot_pair_bars(ax, pairs, colors, label):
    labels = [pair['pair_label'] for pair in pairs]
    scores = [pair['antipodality_score'] for pair in pairs]
    bars = ax.bar(range(len(scores)), scores, color=colors, alpha=0.8, edgecolor='black', linewidth=1)
    ax.set_xlabel('Feature Index Pairs', fontsize=12, fontweight='bold')
    ax.set_ylabel('Antipodality Score', fontsize=12, fontweight='bold')
    confirmed_count = sum(1 for pair in pairs if pair['antipodality_score'] >= 0.8)
    candidate_count = len(pairs) - confirmed_count
    ax.set_title(f'Top {len(pairs)} {label}: {confirmed_count} pairs (>=0.8), {candidate_count} candidates', fontsize=12, fontweight='bold')
    ax.set_xticks(range(len(labels)))
    ax.set_xticklabels(labels, rotation=0, ha='center')
    ax.grid(True, alpha=0.3, axis='y')
    _label_bars(ax, bars, scores)


def plot_within_cross(payload: WithinCrossPayload, out_path: str) -> None:
    """Compare selected within-level and cross-level pairs."""
    ...
    _plot_pair_bars(ax1, payload.top_within, within_colors, 'Within-Level')
    ...
    _plot_pair_bars(ax2, payload.top_cross, cross_colors, 'Cross-Level')
...
def plot_unbiased_antipodal_analysis(payload: dict, out_path: str) -> None:
    """Plot pair-selected cosine matrices with antipodal and dense-pair overlays."""
    _require(payload, [
    ...
    ax1.imshow(C_enc, cmap="RdBu_r", vmin=-1.0, vmax=1.0, aspect="equal")
...
def plot_dense_focused_matrix(payload: dict, out_path: str) -> None:
    """Plot clustered encoder/decoder matrices for the selected dense features."""
    ...
    threshold = payload["antipodal_threshold"]
    ...
    for pair in pairs:
        row, column = pair["matrix_i"], pair["matrix_j"]
        add_pair_rect(ax1, row, column, color="gold", lw=3)
        add_pair_rect(ax2, row, column, color="gold", lw=3)
    ...
    fig.legend(
        handles=[Rectangle((0, 0), 1, 1, fill=False, edgecolor='gold', lw=3,
                           label=f'Antipodal Pairs (score >= {threshold})')],
