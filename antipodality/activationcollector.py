...
def setup_environment():
    """Set the collection seeds and PyTorch precision settings."""
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
...
def main():
    """Collect activation counts and sums for the layer configured in the environment."""
    ...
    # Keep the SAE on CPU to reduce GPU memory use.
    ...
    for _, toks in _iter_batches(texts, batch_size, max_length):
        ...
        resid = store["resid"]
        B, T, Dm = resid.shape
        valid = attn_bool.view(-1).to(resid.device)
        flat = resid.view(B*T, Dm)[valid].contiguous()
        flat_cpu = flat.to('cpu', non_blocking=True) if flat.is_cuda else flat
        acts = sae.encode(flat_cpu)
        pos = acts > 0
        ...
        if seen_tokens >= next_report:
            if model_device.type == "cuda":
                ...
                # Empty the cache every eight progress reports.
